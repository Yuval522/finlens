...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Explicit pre-checks (rather than only relying on the UNIQUE
    // constraint and parsing the resulting SQLite error string) so the
    // user gets a specific, friendly "username taken" vs "email already
    // registered" message instead of a generic failure.
